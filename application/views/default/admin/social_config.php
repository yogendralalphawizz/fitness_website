...
<head>
	...
	.icon_picker_dropdown.show {
		display:inline-block;
		position: absolute;
	}
	
	.icon_picker_dropdown i {
		font-size: 20px;
		cursor:pointer;
		margin:3px;
	}
						...
						<div class="row">
							<div class="col-12">
								<div class="page-title-box d-sm-flex align-items-center justify-content-between">
									<h4 class="mb-sm-0 font-size-18">Social Config</h4>

									<div class="page-title-right">
										<ol class="breadcrumb m-0">
											<li class="breadcrumb-item"><a href="<?php echo base_url('/admin/Dashboard/'); ?>">Dashboard</a></li>
											<li class="breadcrumb-item active">Social Config</li>
										</ol>
									</div>
							...
							<div class="col-md-12">
								<div class="card">
									<div class="card-header groupformheader">
										...
										<h4 class="card-title">Social Config &nbsp; </h4>
										
										<div class="actionbutton">
										    <button type="button" class="btn btn-success waves-effect waves-light" onclick="add_social_row()"> <i class=" fas fa-plus
 font-size-16 align-middle me-2"></i> Add more</button>
									...
									<div class="card-body">
										<form method="post" enctype="multipart/form-data">
											...
											<table class="table table-bordered" id="social_config_table">
												<thead>
													<tr>
														<th>Social name</th>
														<th>Icon class</th>
  ...
  <!-- end  -->
	...
	function icon_picker_init(){
		document.querySelectorAll('.icon_picker:not(.init)').forEach(function(picker){
			picker.classList.add('init');
			
			picker.querySelector('input[type="text"]').addEventListener('focus', function(){
				picker.querySelector('.icon_picker_dropdown').classList.add('show');
			});
			
			picker.querySelectorAll('i').forEach(function(el_i){
				el_i.addEventListener('click', function(){
					picker.querySelector('input[type="text"]').value = el_i.className;
					picker.querySelector('.icon_picker_dropdown').classList.remove('show');
				});
			});
	...
	var i = 0;
	function add_social_row(data = {}){
		for(var item of [ 'platform', 'icon', 'link' ]){
			if( ! (item in data) ){
				data[ item ] = '';
			}
		}
		
		jQuery('#social_config_table tbody').append('<tr> <td> <input type="text" name="socials['+i+'][platform]" class="form-control" placeholder="Platform" required value="'+data.platform+'"> </td> <td> <div class="icon_picker"> <input type="text" name="socials['+i+'][icon]" placeholder="Icon" class="form-control" required value="'+data.icon+'"> <div class="icon_picker_dropdown"> <div class="row"> <?php foreach($fa_icons as $icon){ ?> <div class="col-3"><i class="<?php echo $icon; ?>"></i></div> <?php } ?> </div> </div> </div> </td> <td> <input type="url" name="socials['+i+'][link]" class="form-control" placeholder="Link" required value="'+data.link+'"> </td> <td> <button type="button" class="btn btn-danger" onclick="remove_social_row(this)"><i class="fa fa-trash"></i></button> </td> </tr>');
		i++;
		icon_picker_init()
	}
	
	function remove_social_row(btn){
		jQuery(btn).parents('tr').remove();
	}
	
	<?php foreach($rows as $row){ ?>
	add_social_row(<?php echo json_encode($row); ?>);
	<?php } 
	if( count($rows) == 0 ){ ?>
	add_social_row();
	<?php } ?>
